...
<body>
    <script>
        //     function om(){
        // $("#myModal1").modal('show');}
        function showUpdate(term, id) {
            // $('#myModal1').modal('show');
            //$('#u_ruleSubmit').attr("href", '/ruleDel/' + id)
            document.getElementById('ru1').value = term;
            document.getElementById('update_rules').action = `updatehtml.php?id=${id}`;
        }

        function vdn() {
            let Ru = $("#ru").val();
            if (Ru == "") {
                $("#err").html("please enter rules");
                return false;
            } else if (Ru.length > 100) {
                $("#err").html("Character limit is 100!");
                flag = false;
            }
            if (flag != false) {
        ...
        function vdn1() {
            let Ru = $("#ru1").val();
            if (Ru == "") {
                $("#err1").html("please enter rules");
                return false;
            } else if (Ru.length > 100) {
                $("#err1").html("Character limit is 100!");
                flag = false;
            }
            if (flag != false) {
                        ...
                        <div>
                            <div class="card" style="height: auto;">
                                ...
                                <div class="card-body p-0">
                                    <div>
                                        <h1 style="text-align: center; color: rgb(86, 145, 255);"> Terms & Condition
                                        </h1>
                                        <table style="width: 100%;">
                                            <tr>
                                                ...
                                                <td>
                                                    <button data-toggle="modal" data-target="#myModal" id="mybtn" class="btn btn-light" style="float: right; border-radius: 10px;width:100px;background-color: rgb(86, 145, 255); color: black;">ADD</button>
                                                </td>
                                            </tr>
                                        </table>
                                        <table class="table" id="table-id">
                                            <thead class="bg-lightfor">
                                                ...
                                                <tr class="border-0">
                                                    ...
                                                    <th class="border-0">Terms & Condition</th>
                                                    <!-- <th><button style="border-color: rgb(255, 255, 255);background-color:rgb(39, 132, 238)">ADD</button></th> -->
                                            ...
                                            <tbody>
                                                ...
                                                <?php while ($row = $stmt->fetch(PDO::FETCH_NUM)) {   ?>
                                                    <tr>
                                                        ...
                                                        <td><button class="btn btn-success rounded-sm" style="border-radius: 10px;" onclick="showUpdate('<?php echo $row[0] ?>','<?php echo $row[1] ?>')" id="mybtn1" value="" data-toggle="modal" data-target="#myModal1">UPDATE</button></td>
                                                        <form action="delete.php" method="POST">
                                                            <td>
                                                                <input type="hidden" name="id" value="<?php echo $row[1]; ?>">
                                                                <button class="btn btn-danger rounded-sm" style="border-radius: 10px;" name="deleteTerms">DELETE</button>
                                                            </td>
                                                        </form>
                                                        ...
                                                        <div class="modal-content">
                                                            ...
                                                            <div class="modal-body" style="padding:40px 50px;">
                                                                <form role="form" action="#" method="GET">
                                                                    <div class="form-group">
                                                                        <!-- <label for="usrname"><span class="glyphicon glyphicon-user"></span> <b>Update Question</b></label> -->
                                                                        <input type="text" class="form-control" id="ru" name="term" placeholder="Add a rule">

                                                                    </div>
                                                                    <br>
                                                                    <span id="err" style="font-size: larger; color: red;"></span>
                                                                    <br>
                                                                    <button onclick="return vdn()" type="submit" class="btn btn-success btn-block" style="background-color: #3d405c;" name="submitT"><span class="glyphicon glyphicon-off"></span>Add
                                                                        Rule</button>
                                                            ...
                                                            if (isset($_GET['submitT'])) {
                                                                $term = $_GET['term'];
                                                                $sqlC = "select * from rules where terms = '$term'";
                                                                $stmt = $db->prepare($sqlC);
                                                                $stmt->execute();
                                                                $check = $stmt->rowCount();
                                                                // $data = pg_query($db,$sqlC);
                                                                // $check = pg_num_rows($data);
                                                                if ($check > 0) {
                                                                    echo "<script>alert('Rule already exist')</script>";
                                                                } else {
                                                                    $sql2 = "INSERT INTO rules(terms)VALUES ('$term')";
                                                                    $res = $db->prepare($sql2);
                                                                    $res->execute();
                                                                    if ($res) {
                                                                        echo '<script>alert("Add Rule successfully done")</script>';
                                                            ?>
                                                                        <script>
                                                                            window.location.href = 'http://localhost/Project2/views/term_cond.php';
                                                                        </script>
                                                            <?php
                                                                    } else {
                                                                        echo '<script>alert("Rule not add done")</script>';
                                                                    }
                                                                }
                                                            }
                                                            ?>
                                                ...
                                                <div class="modal fade" id="myModal1" role="dialog">
                                                    <div class="modal-dialog">
                                                        ...
                                                        <div class="modal-content">
                                                            ...
                                                            <div class="modal-body" style="padding:40px 50px;">
                                                                <form role="form" method="POST" id="update_rules" name="update_rules">
                                                                    <div class="form-group">
                                                                        <!-- <label for="usrname"><span class="glyphicon glyphicon-user"></span> <b>Update Question</b></label> -->
                                                                        <input type="text" class="form-control" id="ru1" name="terms" placeholder="Add a rule">
                                                                        <!-- <textarea type="text" class="form-control" id="ru1" name="terms" placeholder="Enter Rule"></textarea> -->
                                                                    </div>
                                                                    <br>
                                                                    <span id="err1" style="font-size: larger; color: red;"></span>
                                                                    <br>
                                                                    <button type="submit" onclick="return vdn1()" class="btn btn-success btn-block" style="background-color: #3d405c;" name="submitTerms"><span class="glyphicon glyphicon-off"></span>Update
                                                                        Rule</button>
                                                                    <button type="reset" class="btn btn-success btn-block" style="background-color: #3d405c;"><span class="glyphicon glyphicon-off"></span>Reset</button>
                                                                </form>
                                                            </div>
